        ...
        with app.app_context():
            for i in range(10):
                ...
                new_event = Event ( name = fake.text(max_nb_chars=50),
                                    ...
                                    image_url = random.choice(user_img_urls),
                                    lng = round(random.uniform(some_Kyiv_cords['x_min'],some_Kyiv_cords['x_max']),6),
                                    lat = round(random.uniform(some_Kyiv_cords['y_min'],some_Kyiv_cords['y_max']),6),
                ...
                # e = Event(name='a', lng=10, lat=20, _start_time=datetime.now(), _end_time=datetime.now(), age_from=10, age_to=100, members_total=15, members_needed=10, sport_id
